...
export class ChatViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'omni.chatView';

    private view?: vscode.WebviewView;
    private extensionUri: vscode.Uri;
    private backendManager: BackendManager;
    private eventBus: EventBus;
    private currentAgent: string = 'assistant';
    ...
    private setupEventListeners(): void {
        ...
        // Handle streaming
        this.eventBus.on(Events.CHAT_STREAM_START, () => {
            this.postMessage({ type: 'streamStart' });
        });

        this.eventBus.on(Events.CHAT_STREAM_CHUNK, (data) => {
            this.postMessage({ type: 'streamChunk', content: data.content });
        });

        this.eventBus.on(Events.CHAT_STREAM_END, () => {
            this.postMessage({ type: 'streamEnd' });
        });

        // Handle errors
        this.eventBus.on(Events.CHAT_ERROR, (data) => {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                ...
                case 'clearHistory':
                    ...
                    break;
                case 'ready':
                    this.eventBus.emit(Events.VIEW_READY);
