...
import { WebSocketClient, Message, WebSocketCallbacks } from '../backend/websocketClient';
  ...
  return jest.fn().mockImplementation((url: string) => {
    const instance = {
      ...
      on: jest.fn((event: string, handler: Function) => {
        (instance as any)[`_${event}Handler`] = handler;
      }),
      send: jest.fn((_data: string, callback?: (err?: Error) => void) => {
      ...
      simulateOpen: () => {
        ...
        (instance as any)._openHandler?.();
      },
      simulateClose: () => {
        instance.readyState = 3; // CLOSED
        (instance as any)._closeHandler?.();
      },
      simulateMessage: (data: any) => {
        const buffer = Buffer.from(JSON.stringify(data));
        (instance as any)._messageHandler?.(buffer);
      },
      simulateError: (error: Error) => {
        (instance as any)._errorHandler?.(error);
      },
    };
    return instance;
  });
...
describe('WebSocketClient', () => {
  ...
  beforeEach(() => {
    ...
    client = new WebSocketClient('ws://localhost:8765', callbacks);
  });
